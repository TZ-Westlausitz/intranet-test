...
import { AufgabeFormFelder, LEERE_AUFGABE_STANDARDWERTE, type AufgabeStandardwerte } from "@/components/aufgabe-form-felder"
 ...
 *
 * "Projekte" (größere, mehrpersonige Vorhaben mit Zeitstrahl) sind
 * bewusst ein eigener, noch nicht gebauter Baustein — siehe Kommentar am
 * Model Aufgabe.
 */
export default async function ToDosSeite({
  ...
  const [{ offen, erledigt }, projektAufgabenOffen] = await Promise.all([
  ...
  return (
    <main className="mx-auto max-w-2xl px-5 py-10">
      ...
      <h1 className="text-center text-2xl font-semibold text-marke-grau md:text-left">To-dos</h1>
        ...
        {offen.length === 0 ? (
          ...
          <ul className="mt-3 flex flex-col divide-y divide-neutral-100">
            {offen.map((aufgabe) => {
              ...
              const standardwerte: AufgabeStandardwerte = {
                titel: aufgabe.titel,
      ...
      {/* Aus Projekten zugewiesene Aufgaben — bewusst nur lesend/verlinkend
          (siehe projektAufgabenFuerPerson): Status ändern, bearbeiten,
          löschen bleibt Sache der Projektseite, damit es dafür nur eine
          Bedienstelle gibt statt zwei parallel gepflegte Listen. */}
        ...
        <div className="mt-4 rounded-xl border border-neutral-200 bg-white p-4">
          ...
          <ul className="mt-3 flex flex-col divide-y divide-neutral-100">
            {projektAufgabenOffen.map((aufgabe) => (
              <li key={aufgabe.id} className="flex items-start justify-between gap-3 py-2.5">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <span
                      className={
                ...
                <Link
                  ...
                  className="shrink-0 text-xs font-medium text-marke-gruen-dunkel hover:underline"
                >
                  Zum Projekt →
                </Link>
              </li>
