      ...
      <div className="flex max-h-[85vh] flex-col">
          ...
          {detail ? (
            <div className="flex min-w-0 items-start gap-2">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-neutral-700">{detail.absenderName}</p>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {laedt || !detail ? (
            ...
            <>
              {detail.kategorie && (
                <span className="inline-block rounded-full bg-marke-gruen/15 px-2 py-0.5 text-[11px] font-medium text-marke-grau">
                  {detail.kategorie.name}
                </span>
              )}

              <h1 className="mt-2 text-xl font-bold text-marke-grau">{detail.titel}</h1>
              ...
              {detail.nochNichtVeroeffentlicht ? (
                <p className="mt-3 flex items-center gap-1.5 rounded-lg bg-marke-orange/15 px-3 py-2 text-sm font-medium text-marke-grau">
...
function UmfrageAnzeige({
  ...
  return (
    <div className="mt-3 flex flex-col gap-1.5">
      {umfrage.frage && <p className="text-sm font-medium text-marke-grau">{umfrage.frage}</p>}
      {umfrage.optionen.map((option) => (
        <label
          key={option.id}
          className="relative flex items-center gap-2 overflow-hidden rounded-lg border border-neutral-200 px-3 py-2 text-sm"
        >
          ...
          <input
            ...
            disabled={nochNichtVeroeffentlicht}
            className="relative shrink-0"
          />
          <span className="relative flex-1">{option.text}</span>
          <span className="relative shrink-0 text-xs text-neutral-500">
            {option.stimmenAnzahl} · {option.prozent}%
          </span>
        </label>
      ))}
      <p className="text-xs text-neutral-400">{umfrage.teilnehmerAnzahl} Teilnehmer:innen</p>
    </div>
  )
}
